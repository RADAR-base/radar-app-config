...
interface Function {
    ...
    suspend fun apply(interpreter: Interpreter, scope: List<Scope>, parameters: List<Expression>): Variable
}
...
class SumFunction : AbstractFunction() {
    ...
    override suspend fun apply(interpreter: Interpreter, scope: List<Scope>, parameters: List<Expression>): Variable =

            parameters.asSequence()
                .flatMap { runBlocking { interpreter.interpret(scope, it).asSequence() } }
                .sumOf { it.asNumber() }
                .toVariable()
}
...
class CountFunction : AbstractFunction() {
    ...
    override val numberOfArguments = 1..Int.MAX_VALUE
    override suspend fun apply(interpreter: Interpreter, scope: List<Scope>, parameters: List<Expression>) = parameters
        .sumOf {
